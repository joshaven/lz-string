...
/*jslint nomen:true white:true plusplus:true, bitwise:true */
/*global Uint8Array, module*/
...
var LZString = {
  ...
  compressToUint8Array: function (uncompressed) {
    'use strict';
    var i, compressed = LZString.compress(uncompressed),
        buf=new Uint8Array(compressed.length*2), // 2 bytes per character
        len=compressed.length, current_value;
  ...
  decompressFromUint8Array:function (compressed) {
    'use strict';
    var buf, result, i, len;
    if (compressed===null || compressed===undefined){
        return LZString.decompress(compressed);
    }
    buf = new Array(compressed.length/2); // 2 bytes per character
    len=buf.length;
    for (i=0; i<len; i+=1) { buf[i]=compressed[i*2]*256+compressed[i*2+1]; }
